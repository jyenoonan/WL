<template>
    <div ref="chatRoot" class="ww-chat-ai" :class="{ 'ww-chat-ai--disabled': isDisabled }" :style="containerStyles">
        ...
        <InputArea
            ...
            :popup-hover-bg-color="popupHoverBgColor"
            :popup-description-color="popupDescriptionColor"
            @send="sendMessage"
...
export default {
    ...
    setup(props, { emit }) {
        ...
        const mentionTools = computed(() => {
            const tools = props.content?.mentionTools;
        ...
        // Mention and popup styles
        const mentionColor = computed(() => props.content?.mentionColor || '#8ab4f8');
        const popupBgColor = computed(() => props.content?.popupBgColor || '#292a2d');
        const popupBorderColor = computed(() => props.content?.popupBorderColor || '#3c4043');
        const popupTextColor = computed(() => props.content?.popupTextColor || '#e8eaed');
        const popupHoverBgColor = computed(() => props.content?.popupHoverBgColor || '#3c4043');
        const popupDescriptionColor = computed(() => props.content?.popupDescriptionColor || '#9aa0a6');
